...
public class preProcess {
	 ...
	 */
	public static void readId(String path,String out){
...
//					Map<String,String> mapTrace=new LinkedHashMap<>();
//					for(int j=0;j<listTrace.size();j++){
//						String value=listTrace.get(j);
//						String[] values=value.split(",",5);
//						String time=values[0];
//						double lat=Double.valueOf(values[1]);
//						double lng=Double.valueOf(values[2]);
//						String v=values[3];
//						String d=values[4];
//						if(v.equals("0")){
//							String b=getXY(bLng,bLat,lng,lat);
//							String[] bb=b.split(",",2);
...
//					}
	...
	public static void getHighWayId(String in,String tollGps,String outPlate){
		double[] a=gps.transform(Double.parseDouble("28.1446523718"), Double.parseDouble("105.3039432470"));
		double bLng=a[1];
		double bLat=a[0];
		Map<String,ArrayList<String>> mapGrid=passStation.getGrid(tollGps);
		Set<String> listHighWayCar=new HashSet<>();
		try{
			File file=new File(in);
			List<String> list=Arrays.asList(file.list());
			for(int i=0;i<list.size();i++){
				...
				if(fileIn.getName().endsWith(".csv")){
					...
					while((line=reader.readLine())!=null){
						...
						String v=data[3];
						String d=data[4];
						if(v.equals("0")&&!gps.equals(lat+","+lng)){
							gps=lat+","+lng;
							String xy=passStation.getXY(Double.valueOf(lng),Double.valueOf(lat),50);
							ArrayList<String> isPassingToll=passStation.isPassToll(xy,mapGrid);
							if(isPassingToll!=null){
								listHighWayCar.add(id);
								break;
							}
						}
					}
			...
			System.out.println("经过高速的车辆："+listHighWayCar.size());
			BufferedWriter writer=io.getWriter(outPlate, "gbk");
			for(String id:listHighWayCar){
				writer.write(id+"\n");
			}
			writer.flush();
			writer.close();
...
//		getHighWayId(cqPlateAllTrace1,outPath18Chongqing,cqPlateHighWay);
		
//		getPassStation(cqPlateAllTrace1,outPath18Chongqing,cqPassStation);//得到经过收费站的车辆id，经过的收费广场经纬度，经过的时间
