...
public class matchData {
	...
	public static void getMatchedData(String matchedId,String cqPassStation,String cqTollDataOut) throws ParseException{
		Map<String,String> mapMatchedId=readMatchedId(matchedId);
		Map<String,String> mapIdOriginTrace=readCqPassStation(cqPassStation,mapMatchedId);
		Map<String,String> mapCarTrace=readTrace(cqTollDataOut,mapMatchedId);
		
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		int count=0;
		Map<String,String> mapNotAllMatch=new HashMap<>();
		Map<String,String> mapMatchedData=new HashMap<>();
		for(String id:mapIdOriginTrace.keySet()){
			...
			String newCarTrace="";
			while(i<carTraces.length&&j<gpsTraces.length){
				String carStation=carTraces[i].split(",")[0];
				String carTime=carTraces[i].split(",")[1];
				String carWeight=carTraces[i].split(",")[2];
				String gpsTime=gpsTraces[j].split(",")[2];
				boolean flag=dataRepair1.isMatch(carTime,gpsTime,3);
				if(flag){
					i++;
					j++;
					newCarTrace+=carStation+","+gpsTime+","+carWeight+"|";
				}else{
			...
			if(carTraces.length==newCarTrace.split("\\|").length){
				count++;
				String[] newCarTraces=newCarTrace.split("\\|");
				String inAndOutData="";
				for(int k=0;k<newCarTraces.length;k+=2){
					String[] in=newCarTraces[k].split(",");
					inAndOutData+=in[0]+","+in[1]+","+newCarTraces[k+1]+"|";
				}
				inAndOutData=inAndOutData.substring(0, inAndOutData.length()-1);
				mapMatchedData.put(id+","+car, inAndOutData);
			}else{
				mapNotAllMatch.put(id+","+car, carTrace);
			}
//			System.out.println(id+","+gpsTrace);
//			System.out.println(carTrace);
//			System.out.println(newCarTraces);
		}
		
		System.out.println("长度相等条数:"+count);
			
	}
//	Map<String,String> mapNotAllMatch,Map<String,String> mapMatchedData
	...
	public static void processOneId(String id,String path,Map<String,String> mapMatchedId,Map<String,String> mapCarTrace,Map<String,ArrayList<String>> mapServiceXy,Map<String,String> mapMatchedData) throws ParseException{
		...
		while(b<carTraces.length&&c<listGps.size()){
			...
			boolean flag=dataRepair1.isMatch(carTime, gpsTime, 1);
			if(flag){
				matchedCount++;
				int t=c;
				if(b%2==0){
					...
					while(t+1<listGps.size()&&flagT){
						...
						flagT=dataRepair1.isMatch(carTime, gpsTime, 1);
						t++;
					}
					if(serviceId.equals("")){
...
//		getMatchedData(matchedId,cqPassStation,cqTollDataOut);
